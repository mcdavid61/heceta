...
/*
   Function:  EEPROM_GetFailsafeRelayEnable()
   Description:
    Returns the present FailsafeRelayEnable, as we believe
    it is stored in the EEPROM.
 */
uint16_t EEPROM_GetFailsafeRelayEnable(void)
{
  return EEPROM_Ready() ? m_sEEPROMConfiguration.nFailsafeRelayEnable : 0;
}

/*
   Function:  EEPROM_SetFailsafeRelayEnable()
   Description:
    Sets the FailsafeRelayEnable, and the dirty bit.
 */
void EEPROM_SetFailsafeRelayEnable(uint16_t nFailsafeRelayEnable)
{
  if (EEPROM_Ready())
  {
    m_sEEPROMConfiguration.nFailsafeRelayEnable = nFailsafeRelayEnable;
    EEPROM_MarkConfigurationAsDirty();
  }
}
